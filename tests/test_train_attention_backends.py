...
def test_find_window_backend_from_decoder_block_attn():
    model = _build_orion(AttentionConfig(backend="window", window_size=8))
    sparse_backend, dense_backend, window_backend = _find_attention_backends(model)

    assert sparse_backend is None
    assert dense_backend is None
    assert window_backend is not None
